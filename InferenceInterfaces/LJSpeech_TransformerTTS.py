...
class LJSpeech_TransformerTTSInference(torch.nn.Module):
    ...
    def __init__(self, device="cpu", speaker_embedding=None):
        ...
        # try:
        self.phone2mel = Transformer(path_to_weights=os.path.join("Models", "best_models", "best_reformer_3xlong.pt"),
                                         idim=166, odim=80, spk_embed_dim=None, reduction_factor=2).to(torch.device(device))
        # except RuntimeError:
        #     self.phone2mel = Transformer(path_to_weights=os.path.join("Models", "best_models", "best_reformer_3xlong.pt"),
        ...
        self.mel2wav = HiFiGANGenerator(path_to_weights=os.path.join("Models", "hifigan_best.pt")).to(torch.device(device))
        self.phone2mel.eval()
        self.mel2wav.eval()
